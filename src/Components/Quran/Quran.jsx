...
const Quran = () => {
  ...
  const getAyat = (id) => {
    ...
    axios
      .get("https://api.quran.gading.dev/surah/" + id)
      .then((res) => {
        setAuautsoura({
          ...auatSoura,
          ayat: res.data.data.verses,
          loading: false,
        });
      })
      .catch((err) => {
  ...
  const getSoundAyat = async (id) => {
    setAduioAyat({ ...audioAyat, loading: true });
    await axios
      ...
      .then((res) => {
        setAduioAyat({
          ...
          soundAya: res.data,
          loading: false,
          urlSound: res.data.reciters[8].moshaf[1].server,
        });
      })
      .catch((err) => {
        console.log("error");
                  ...
                  {!audioAyat.loading && soundLink != "" ? (
                    <>
                      <div className="surahSound">
                        <i
                          class="fa-regular fa-rectangle-xmark closeSound"
                          onClick={() => {
                        ...
                        return (
                          <>
                            <span className="oneaya">
                              ...
                              <span className="number-aya speColor">
                                {" "}
                                ({numberofAya})
                              </span>
                            </span>
                      ...
                      <>
                        <>
                          <div className="reader reader-notfound">
                            <h2 className="">
                              لا يوجد ســــوره بهذا الاســــم{" "}
                            </h2>
                          </div>
                        </>
                      </>
                          ...
                          return (
                            <>
                              <div className="surah">
                                ...
                                {sora.revelationType === "Meccan" ? (
                                  <p className="loon mb-1">مكية</p>
                                ) : (
                                  <p className="loon mb-1">مدنية</p>
                                )}
                                <div className="surah-icons">
                                  <a href="#SOWARelQURAN" title="قراءة السورة">
                                    <i
                                      class="fa-brands fa-readme read-icon "
                                      onClick={() => {
                                        getAyat(sowrList.indexOf(sora) + 1);
                                        setShowayatbox(true);
                                      }}
                                    ></i>
                                  </a>
                                  <a
                                    href="#SOWARelQURAN"
                                    title="استمع الي الصورة بصوت الشيخ محمود علي البنا"
                                  >
                                    <i
                                      class="fa-solid fa-circle-play listen-icon "
                                      onClick={() => {
                                        getSoundAyat(
                                          sowrList.indexOf(sora) + 1
                                        );
