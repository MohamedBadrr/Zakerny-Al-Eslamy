...
const HasnElmoslem = () => {
      ...
      useEffect(()=>{
        ...
        .catch((err)=>{
            setAzkar({...azkar,loading:false , err:"some thing went wrong ,please try again later"});
        })
    },[azkar.reload])
    ...
    const searchReaders = () => {
      ...
      if (!azkar.results || !azkar.results.length) {
        return; // Prevent search when no readers are loaded yet
      }
      // Replace the following with your own logic to implement delay
      ...
      setTimeout(() => {
        ...
        setSearchAzkarsList({ ...searchAzkarList, results: filteredReaders ,loading:false  });
    ...
    const zkrWithId=async(id)=>{
      ...
      await axios.get(`https://www.hisnmuslim.com/api/ar/${id}.json`).
        then((resp)=>{
              setOnezkar({...onezkar,loading:false,results:Object.values(resp.data)[0]})
        })
        .catch((err)=>{
        })
    }
    ...
    const englishToArabic =(number) => {
      // Define a regular expression to match English digits.
      const englishDigits = /\d/g;
      // Define a dictionary mapping English digits to their Arabic equivalents.
      const arabicDigits = {
    ...
    <>
      <div>
        ...
        <div className='title-azkar'>
            ...
            <h3 className='mt-3 mb-3 text-center'>حـــــصـــــن المـــــســـــلـــــم </h3>
            ...
            <form className='mt-4 text-center'>
                <div class="form-search m-auto">
                    ...
                    value={searchValue} onChange={(e)=>{setSearchValue(e.target.value)}}    placeholder="  البـــــحـــــث....."/>
                        ...
                        (!searchAzkarList.loading)?<>
                                {
                                    ...
                                    <>
                                                ...
                                                <div className='zkr reader-notfound' >
                                                    <h2 className=''>لا يــــوجد ذٍكــــرُ بــــهــــذا الاســــم </h2>
                                                </div>
                                                </>
                                    </>
                                    :
                                    <>
                                    {
                                        searchAzkarList.results.map((zkr,index)=>{
                                            return(
                                                <>
                                                <div className='zkr' >
                                                    <a href="#ZKRR" onClick={()=>{zkrWithId(zkr.ID); setTitleZkr(zkr.TITLE); setShowboxcont(true);}}><h4 className="zkr-title">{zkr.TITLE}</h4></a>
                                                </div>
                                                </>
                                            )
                                    })
        ...
        {
          (showboxcontent && (<div className="" id="ZKRR">
          <div className="container oneZkrContainer  p-4">
          <i class="closeicon fa-regular fa-rectangle-xmark" onClick={()=>{setShowboxcont(false);setShowSoundboxcont(false)}}></i>
                <div>
                    ...
                    </div>
                    {
                      onezkar.results.map((one)=>{
                        ...
                        <div className="zkrContent" >
                              ...
                              <p className=" text-dark">عدد مرات التكرار :<span className="number">{numberofAya}</span></p>
                              {(!onezkar.loading)?<>
                                {(showSoundboxcontent&&(<audio src={one.AUDIO} controls></audio>))}
                                </>
                                  :
                                  <>
                                    <i class="fa-solid fa-spinner fa-spin loadingSound"></i>
                                  </>}
                              <i class="fa-solid fa-circle-play play-icon" onClick={playSound}></i>
        ...
        <button className='up-button'><a href="#LISTAZKAR"><i class="fa-solid fa-chevron-up"></i></a></button>               
    </div>
    <Footer />
    </>
  );
